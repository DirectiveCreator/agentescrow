...
export default function Dashboard() {
  ...
  const [activeSection, setActiveSection] = useState<'overview' | 'hire' | 'board' | 'events' | 'architecture' | 'delegation' | 'ens' | 'filecoin' | 'summary' | 'build-story' | 'join'>('overview');
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg-main)', color: 'var(--text-primary)' }}>
      ...
      <nav className="border-b" style={{ borderColor: 'var(--border)' }}>
        <div className="max-w-7xl mx-auto px-6 flex gap-0">
          {(['overview', 'hire', 'board', 'events', 'architecture', 'delegation', 'ens', 'filecoin', 'summary', 'build-story', 'join'] as const).map(section => (
            <button
              key={section}
